...
# Read nodes
R.tReadNodes(fnSH[instances],fnSF[instances])
#Read branches
R.tReadBranches(fbSH[instances],fbSF[instances])
#Read generators
R.tReadGenerators(fgSH[instances],fgSF[instances])
# nodes
n=R.cdfDataNodes
# branches
k=R.cdfDataBranches
...
def PVS_rule(model):
    ...
    for i in n:
        if i['busType'] in[2,3] :
            retval.append(i['cdfNum'])
    return retval
model.PVSbuses=Set(initialize=SWING_rule,doc='SWING busses')
# Branches
...
def InitConnActive(model,node):
    retval=[]
    for j in k:
        if j['fromBus']==node and j['TR'] != 0 and j['cdfNum'] in model.branches:
            retval.append(j['cdfNum'])
    return retval
...
def populateBranches_11(model,branch):
    for i in k:
        if i['cdfNum']==branch:
            if i['X'] ==0:
                return 999
            return model.BranchG[branch] *0.5
...
#model.xcV   =Var(model.nodes, initialize=init_xcv , domain=Reals , doc='Voltage magnitude on each node (p.u)' )
...
model.xQg   =Var(model.nodes, domain=NonNegativeReals,doc='Reactive power generation - state variable (MVAr)')
...
model.cA    =Var(model.branches, domain=NonNegativeIntegers , bounds=(0,None), doc='control var- transformer branches                                                                                 params t=a exp^(jV)- decide on a (p.u)                                                                                =1 for non-active branches' )
...
model.cB    =Var(model.branches, domain=NonNegativeReals, doc='control var- susceptance of active branch (p.u)' )
...
model.cG    =Var(model.branches, domain=NonNegativeReals, doc='control var- admittance of active branch (p.u)' )
#generators
def cPgjlimits(model,gen):
    return (0,model.GenPMAX[gen])
#model.cPgj   =Var(model.generators, domain=cPgjlimits, bounds=cPgjlimits,doc='Active power of each generator - control variable (MW)')
model.cPgj   =Var(model.generators, domain=NonNegativeReals,doc='Active power of each generator - control variable (MW)')
...
model.xQgj   =Var(model.generators, domain=Reals,doc='Reactive power of each generator - control variable (MW)')
...
try:
    ...
    model.FixPassiveBranch_2.deactivate()
except:
    pass    
...
try:
    ...
    model.FixPassiveBranch_3.deactivate()
except:
    pass 
...
# Generation in a node is the sum of the generators output
...
model.sumGensActive=Constraint(model.nodes, rule=InitSumGensActive, doc='Sum Active Generation per node')
...
model.sumGensReactive=Constraint(model.nodes, rule=InitSumGensReactive, doc='Sum Active Generation per node')
...
# print ('PQ:',len(model.PQbuses))
# print ('PV:',len(model.PVbuses))
# print ('Slack:',len(model.SWINGbuses))

# print ('branches:',len(model.branches))
# print ('Active branches:',len(model.ActiveBranches))
# print ('Passive branches:',len(model.PassiveBranches))

# print('generators:',len(model.generators))
